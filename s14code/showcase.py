...
def build_corpus_dashboard(run: dict) -> dict:
    ...
    # Header
    add({"id": "title", "type": "Heading", "text": {"$bind": "/kpi_title"}})
    ...
    add({"id": "kpi_row", "type": "Grid", "cols": 4, "children": kpi_ids})

    # Charts row
    add({"id": "bars", "type": "BarChart", "title": "Chunks per paper", "data": {"$bind": "/chunks_by_paper"}, "xKey": "label", "yKey": "value", "unit": "chunks"})
    ...
    add({"id": "charts_row", "type": "Grid", "cols": 3, "children": ["bars", "spark", "prog"]})

    # Sortable / filterable data table
    add({"id": "table", "type": "DataTable", "columns": "Paper,Chunks,Words,Year", "rows": {"$bind": "/table_rows"}, "sortable": True, "filterKey": "Paper"})

    # Tabbed drill-downs (one Tab per paper)
    tab_ids = []
    for i, _p in enumerate(papers):
        detail_id = add({"id": f"detail_{i}", "type": "Text", "text": {"$bind": f"/paper_{i}_detail"}})
        tab_ids.append(add({"id": f"tab_{i}", "type": "Tab", "label": f"paper {i + 1}", "children": [detail_id]}))
    add({"id": "tabs", "type": "Tabs", "labels": "one,two,three,four,five", "children": tab_ids})
