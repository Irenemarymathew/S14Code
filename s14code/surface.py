...
def build_run_surface(snapshot: dict) -> dict:
    ...
    components.append({"id": "h_title", "type": "Heading", "text": {"$bind": "/title"}})

    # One card row per node: name badge + state badge.
    for i, (nid, node) in enumerate(sorted(nodes.items())):
        ...
        components.append(
            {"id": row_id, "type": "Row", "children": [name_id, state_id]}
        )
        components.append({"id": name_id, "type": "Text", "text": {"$bind": f"/node_{i}_name"}})
        components.append(
            {"id": state_id, "type": "Badge", "text": {"$bind": f"/node_{i}_state"},
